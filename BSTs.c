...
BST_Node *BST_insert(BST_Node *root, BST_Node *new_node)
{
    ...
    // use < rather than <= because if its equal we will do nothing
    if (new_node -> key < root -> key)
...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
    ...
    double target_key = (10.0 * bar) + index;
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
    ...
    if (root -> key == target_key) {
        ...
        if (root -> left == NULL && root -> right == NULL) {
        ...
        } else if (root -> left == NULL) { // only right node exists
            ...
            // copy data of successor
            root -> key = temp -> key;
            root -> freq = temp -> freq;
            root -> bar = temp -> bar;
            root -> index = temp -> index;

            // delete successor and reset the right node just in case the successor is directly to right
...
void delete_BST(BST_Node *root)
{
    ...
    // we need post order to be able to traverse tree then free
...
int findMinBar(BST_Node *root) {
    ...
    if (root == NULL)
        return -1; // some value that won't tangibly exist as a bar

    int left_min_bar = findMinBar(root -> left);
    int right_min_bar = findMinBar(root -> right);
    int bar = root -> bar;

    if (left_min_bar < bar && left_min_bar != -1)
        bar = left_min_bar;

    if (right_min_bar < bar && right_min_bar != -1)
        bar = right_min_bar;
...
int findMaxBar(BST_Node *root) {
    ...
    int left_max_bar = findMaxBar(root -> left);
    int right_max_bar = findMaxBar(root -> right);
    int bar = root -> bar;

    if (left_max_bar > bar)
        bar = left_max_bar;

    if (right_max_bar > bar)
        bar = right_max_bar;
...
void *reverseTree(BST_Node *root, int max, int min) {

    // we will invert the tree and edit the values
    ...
    reverseTree(left, max, min);
    reverseTree(right, max, min);

    int new_bar = min + max - root -> bar;

    double new_index = 1.0 - root -> index;
    double new_key = (10.0 * new_bar) + new_index;
...
BST_Node *reverseSong(BST_Node *root)
{
    ...
    reverseTree(root, findMaxBar(root), findMinBar(root));
...
double getNewFrequency(double freq, int semitones) {

    for (int i = 0; i < 100; i++) {
        if (freq == note_freq[i]) {
            int index = i + semitones;
            if (index >= 0 && index <= 99) {
    ...
    return -1.0; // no frequency offset?

}

void traverseAndInsert(BST_Node *head, BST_Node *root, int semitones, double time_shift) {

    if (root == NULL)
        return;

    traverseAndInsert(head, root -> left, semitones, time_shift);
    traverseAndInsert(head, root -> right, semitones, time_shift);

    double new_frequency = getNewFrequency(root -> freq, semitones);
    ...
    BST_insert(head, newBST_Node(new_frequency, root -> bar, new_index));
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
    ...
    traverseAndInsert(root, root, semitones, time_shift);
