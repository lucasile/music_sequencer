...
#include "BSTs_1008027298.c"

void num_elements(BST_Node *root, int *count) {

    if (root == NULL)
        return;

    num_elements(root -> left, count);

    *count = *count + 1;

    num_elements(root -> right, count);
...
int main()
{
    ...
    // test min
    int min = findMinBar(root);
    printf("Min Bar: %d\n", min);

    for (int i = 0; i < 100; i++) {
      BST_insert(root, newBST_Node(note_freq[i], i, (1e-6*(double)rand()/(double)RAND_MAX)-(.5e-6)));
    }

    int count = 0;
    int *p_count = &count;

    num_elements(root, p_count);

    printf("\n+-----Checking-Harmonize-----+\n\n");

    printf("Base Notes: %d\n\n", *p_count);

    BST_harmonize(root, 2, 0.05);

    *p_count = 0;

    num_elements(root, p_count);

    printf("Harmonized Notes: %d\n\n", *p_count);
